...
- (void)initialize
{
    ...
    NSArray *weekSymbols = [NSArray arrayWithObjects:@"S",@"M",@"T",@"W",@"T",@"F",@"S", nil];
    ...
    collectionViewFlowLayout.itemSize = CGSizeMake(2, 2);
...
- (void)layoutSubviews
{
    ...
    _supressEvent = YES;
    CGFloat padding = self.fs_height * 0.02;
...
{
    FSCalendarCell *cell1 = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
      cell1.date = [self dateForIndexPath:indexPath];
      NSDate *dates=cell1.date;

   
    [[SingletonClass singleton].calenderDate setObject:@{ @"Date": dates} forKey:@"date"];
