...
@implementation recommend1
@synthesize bundelID;

- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if (IS_IPAD) {
        self = [super initWithNibName:@"recommend1_ipad" bundle:nibBundleOrNil];
    }
    else if(IS_IPHONE_5_OR_MORE) {
        
        self = [super initWithNibName:@"recommend1" bundle:nibBundleOrNil];
    }
    else {
        self = [super initWithNibName:@"recommend1_4" bundle:nibBundleOrNil];
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    [Globals GoogleAnalyticsScreenName:@"RECommend 1"];
    count=0;
...
- (void)viewWillAppear:(BOOL)animated {
    [SingletonClass singleton].cartSelectedProduct=nil;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if ([bundelID containsObject:[[arrayRespose objectAtIndex:section]valueForKey:@"BundleID"]]) {                UIView *views=[[UIView alloc]initWithFrame:CGRectMake(0, 0,320,headerView.frame.size.height)];
                [views setBackgroundColor:[UIColor colorWithRed:233/255.f green:233/255.f blue:233/255.f alpha:0.50f]];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    [headerView setBackgroundColor:[UIColor colorWithRed:233/255.0f green:233/255.0f blue:233/255.0f alpha:1.0]];
    return headerView;
...
- (void) editBundle:(UIButton *)senderBtn {
    ...
    if (![NSStringFromClass([viewContrllss class]) isEqualToString: @"recommend"]) {
        editing.isClient=YES;
    }
    editing.bundelName=[[arrayRespose objectAtIndex:senderBtn.tag]valueForKey:@"Name"];
...
-(void)callDataFormServer
{
    ...
    NSDictionary *inputDic=[NSDictionary dictionaryWithObjectsAndKeys:trainerID,@"trainer_id",@"1",@"bundles_only" ,nil];
    ...
    [manager POST:urlString parameters:inputDic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (json !=nil && json.allKeys.count!=0) {
         ...
         //     [self loadProductToProducts:[json objectForKey:@"Recommendations"]];
                ...
                _errorLabel.text=@"No Recommentations yet";
                [hudFirst hide:YES];
            }
        }
        else {
