...
@synthesize sent, messageLabel, messageView, timeLabel, userImage, avatarImageView, balloonView;
...
+(UIImage*)balloonImage:(BOOL)sent isSelected:(BOOL)selected {
    if (sent == YES && selected == YES) {
        ...
        image=[UIImage imageNamed:@"balloon_unread_right1"];
//        image. = [UIColor blueColor];
        return [image stretchableImageWithLeftCapWidth:24 topCapHeight:15];
        
    } else if (sent == YES && selected == NO) {
        return [[UIImage imageNamed:@"balloon_unread_right1"] stretchableImageWithLeftCapWidth:24 topCapHeight:15];
...
-(id)initMessagingCellWithReuseIdentifier:(NSString*)reuseIdentifier {
    if (self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier]) {
       ...
       ;
        ...
        userImage = [[UIImageView alloc] initWithImage:nil];
...
- (void)layoutSubviews {
    ...
    CGRect userImageFrame= CGRectZero;
    
    if (self.sent == YES) {
        timeLabelFrame = CGRectMake(self.frame.size.width - dateSize.width - textMarginHorizontal, 0.0f, dateSize.width, dateSize.height);
        
        ballonViewFrame = CGRectMake(240 - (textSize.width + 2*textMarginHorizontal)+25, timeLabelFrame.size.height+20, textSize.width + 2*textMarginHorizontal, textSize.height + 2*textMarginVertical);
        
        messageLabelFrame = CGRectMake(235 - (textSize.width + textMarginHorizontal)+25,  ballonViewFrame.origin.y + textMarginVertical, textSize.width, textSize.height);
        
        userImageFrame = CGRectMake(265, 5, 50.0f, 50.0f);
        
        avatarImageFrame = CGRectMake(265, 5, 51.0f, 51.0f);
        ...
        ballonViewFrame = CGRectMake(55, timeLabelFrame.size.height+20, textSize.width + 2*textMarginHorizontal, textSize.height + 2*textMarginVertical);
        
        messageLabelFrame = CGRectMake(textMarginHorizontal+55, ballonViewFrame.origin.y + textMarginVertical, textSize.width, textSize.height);
        
        
        userImageFrame = CGRectMake(5, 5, 50, 50);
        
        avatarImageFrame = CGRectMake(5, 5, 51.0f, 51.0f);
